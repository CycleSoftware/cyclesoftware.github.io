...
| `data.result_items[].stock_stores[].quantity`                          | `integer`  | Sum of available quantity `new`+`demo`+`used`+`rental` e.g. `4` (quantity of objects have priority over article stock if both are present)                                                                    |
